...
cd wayline
git checkout updates        # <-- always work on this branch
...
|------|-------------------|---------------|
| `wayline` | `updates` | api-gateway, docker-compose, postgres, docs |
...
```bash
git checkout updates              # make sure you're on the right branch
git pull origin updates           # get everyone else's latest work
```
...
### Step 6 — Open a Pull Request into `updates`
...
git pull origin updates             # get your merged work + anything else
...
The error `! [rejected] updates -> updates (fetch first)` means:
**"You didn't pull before pushing."**

Two rules eliminate it completely:
1. **Always `git pull origin updates` before starting work** (Step 2 above)
2. **Work on feature branches, not directly on `updates`** (Step 3 above)
...
When `updates` has been tested and is ready for a stable release:

```bash
# On GitHub — open a PR from updates → main
gh pr create \
  --base main \
  --head updates \
...
Resolve the two known conflicts (they will appear because `main` still has the old monolith):
- `public/index.css` — accept the deletion (our side)
- `public/index.html` — accept the deletion (our side)

Merge the PR on GitHub. Do not merge locally.

---

## 7. Enable Branch Protection (Project Lead — Do This Once)

Right now `main` has no protection rules enforced on GitHub.
The README says it's protected — make GitHub actually enforce it:

1. Go to [github.com/YUVARAJ-R-ai/wayline/settings/branches](https://github.com/YUVARAJ-R-ai/wayline/settings/branches)
...
Do the same for `wayline-nextjs`.
...
```bash
git pull origin updates     # or your branch name
git push origin updates
```

### "Divergent branches" on pull
```bash
git pull --no-rebase origin updates   # explicit merge strategy
```

### "I committed to `updates` directly instead of a feature branch"
...
### "My feature branch is very out of date with `updates`"
```bash
git checkout updates
git pull origin updates
git checkout feature/your-branch
git merge updates             # bring updates into your feature branch
...
git checkout updates && git pull origin updates
